!function() {
    function e(e = "") {
        ...
        let a = n.domain;
        if (a && /^\/.+\//.test(a)) {
            const e = new RegExp(a.slice(1, -1)).exec(s.hostname);
            a = e ? e[0] : void 0;
        }
        const c = () => {
            o.String_split.call(document.cookie, ";").forEach((e => {
                ...
                const n = o + "=", i = `; domain=${s.hostname}`, c = `; domain=.${s.hostname}`;
                let d, l;
                if (a) d = `; domain=${a}`, l = `; domain=.${a}`; else if (document.domain) {
                    const e = document.domain;
                    e !== s.hostname && (d = `; domain=.${e}`), e.startsWith("www.") && (l = `; domain=${e.replace("www", "")}`);
                }
                const g = "; path=/", p = "; Max-Age=-1000; expires=Thu, 01 Jan 1970 00:00:00 GMT";
                document.cookie = n + p, document.cookie = n + i + p, document.cookie = n + c + p, 
                document.cookie = n + g + p, document.cookie = n + i + g + p, document.cookie = n + c + g + p, 
                void 0 !== d && (document.cookie = n + d + g + p), void 0 !== l && (document.cookie = n + l + g + p);
            }));
        };
        if (c(), window.addEventListener("beforeunload", c), "string" != typeof n.when) return;
        const d = [ "scroll", "keydown" ], l = o.String_split.call(n.when, /\s/);
        for (const e of l) !1 !== d.includes(e) && document.addEventListener(e, (() => {
            i(c);
        }), {
            passive: !0
        });
    }
    const t = {}, o = [ [ "/^bda|^bltsr/" ], [ "/^bltsr$|^JPIqApiY$|^specific$|^substantial$/" ], [ "/adblock_/" ], [ "hurricane" ], [ "isab" ], [ "shedevrum-aab" ], [ "yadb" ] ], r = new Map([ [ "comedy-radio.ru", 0 ], [ "kufar.by", 0 ], [ "radioromantika.ru", 0 ], [ "relax-fm.ru", 0 ], [ "rg.ru", 0 ], [ "sm.news", 0 ], [ "ura.news", 0 ], [ "veseloeradio.ru", 0 ], [ "yandex.by", 1 ], [ "yandex.kz", 1 ], [ "yandex.ru", 1 ], [ "yandex.uz", 1 ], [ "yapx.ru", 2 ], [ "drive2.ru", 3 ], [ "24smi.org", 4 ], [ "shedevrum.ai", 5 ], [ "kakprosto.ru", 6 ] ]), n = new Map([ [ "passport.yandex.by", [ 1 ] ], [ "passport.yandex.kz", [ 1 ] ], [ "passport.yandex.ru", [ 1 ] ], [ "passport.yandex.uz", [ 1 ] ] ]), i = (e, t, o) => {
        let r = t.get(e);
    ...
    for (const t of c) if (!d.has(t)) try {
        e(...o[t]);
    } catch {}
}();