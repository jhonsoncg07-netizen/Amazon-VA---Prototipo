!function() {
    function e(e = "", o = "", r = "") {
        const n = t(), i = n.makeLogPrefix("trusted-click-element", e, o, r);
        if ("" !== o) {
            const e = n.String_split.call(o, ",").map((e => {
                const t = e.indexOf(":"), o = -1 !== t ? e.slice(0, t) : e, r = o.startsWith("!"), n = r ? o.slice(1) : o, i = -1 !== t ? e.slice(t + 1).trim() : "";
                if ("" === i) return;
                const s = {
                    not: r,
                    type: n
                }, a = /^\/(.+)\/(i?)$/.exec(i);
                if (null !== a) return s.re = new RegExp(a[1], a[2] || void 0), s;
                const c = i.indexOf("="), l = -1 !== c ? i.slice(0, c).trim() : i, d = -1 !== c ? i.slice(c + 1).trim() : "";
                return s.re = new RegExp(`^${this.escapeRegexChars(l)}=${this.escapeRegexChars(d)}`), 
                s;
            })).filter((e => void 0 !== e)), r = e.some((e => "cookie" === e.type)) ? t().String_split.call(document.cookie, /\s*;\s*/).map((e => {
                const t = e.indexOf("=");
                if (0 !== t) return -1 === t ? `${e.trim()}=` : {
                    key: e.slice(0, t).trim(),
                    value: e.slice(t + 1).trim()
                };
            })).filter((e => void 0 !== e)) : [], i = e.some((e => "localStorage" === e.type)) ? function(e = "localStorage") {
                const t = self[e];
            ...
            for (const {not: t, type: o, re: n} of e) switch (o) {
              case "cookie":
                if (s(r, n) === t) return;
                break;
        ...
        const s = (e, t = document) => {
            const o = e.indexOf(" >>> ");
            if (-1 === o) return t.querySelector(e);
            const r = e.slice(0, o).trim(), n = e.slice(o + 5).trim(), i = t.querySelector(r);
            if (null === i) return null;
            const a = (e => e.openOrClosedShadowRoot ? e.openOrClosedShadowRoot : "object" == typeof chrome && chrome.dom && chrome.dom.openOrClosedShadowRoot ? chrome.dom.openOrClosedShadowRoot(e) : null)(i);
            return a && s(n, a);
        }, a = n.String_split.call(e, /\s*,\s*/).filter((e => {
            try {
                s(e);
            } catch {
                return !1;
        ...
        const c = parseInt(r, 10) || 1, l = Date.now(), d = l + 1e4;
        let u = 1 !== a.length ? l : l + c;
        const g = () => {
            a.length = 0, p.stop(), m.stop();
        }, p = e => {
            if (0 === a.length) return n.uboLog(i, "Completed"), g();
            const t = Date.now();
            if (t >= d) return n.uboLog(i, "Timed out"), g();
            e && m();
            const o = Math.max(e ? d - t : u - t, 1);
            p.timer = setTimeout((() => {
                p.timer = void 0, f();
            }), o), n.uboLog(i, `Waiting for ${a[0]}...`);
        };
        p.stop = () => {
            void 0 !== p.timer && (clearTimeout(p.timer), p.timer = void 0);
        };
        const m = () => {
            void 0 === m.observer && (m.observer = new MutationObserver((() => {
                void 0 === m.timer && (m.timer = setTimeout((() => {
                    m.timer = void 0, f();
                }), 20));
            })), m.observer.observe(document, {
                attributes: !0,
                childList: !0,
                subtree: !0
            }));
        ...
        m.stop = () => {
            void 0 !== m.timer && (clearTimeout(m.timer), m.timer = void 0), m.observer && (m.observer.disconnect(), 
            m.observer = void 0);
        };
        const f = () => {
            if (p.stop(), Date.now() < u) return p();
            const e = a.shift();
            if (void 0 === e) return g();
            const t = s(e);
            if (null === t) return a.unshift(e), p(!0);
            n.uboLog(i, `Clicked ${e}`), t.click(), u += c, p();
        };
        !function(e) {
